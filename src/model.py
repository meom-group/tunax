...
class SingleColumnModel(eqx.Module):
    ...
    def step(self, state: State, closure_state: ClosureStateAbstract,
             closure_parameters: ClosureParametersAbstract,
             swr_frac: jnp.ndarray) -> Tuple[State, ClosureStateAbstract]:
        ...
        u_new, v_new = advance_dyn_cor_ed(
            ...
            closure_state.akv, self.case.fcor, self.dt)
    ...
    def compute_trajectory_with(self, closure_parameters: ClosureParametersAbstract) -> Trajectory:
        ...
        trajectory = Trajectory(
            self.grid, time,
            jnp.vstack(t_list), jnp.vstack(s_list), jnp.vstack(u_list), jnp.vstack(v_list))
        return trajectory
