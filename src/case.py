...
class Case(eqx.Module):
    ...
    alpha: float = 2e-4                         # thermal expansion coefficient [C-1]
    beta: float = 2e-4                          # salinity expansion coefficient [psu-1]

    # forcings
    fcor: float = 0.                            # Coriolis frequency [rad.s-1]
    ustr_sfc: float = 0.                        # zonal wind stress [m2.s-2]
    ustr_btm: float = 0.                        # zonal current stress at the bottom [m2.s-2]
    vstr_sfc: float = 0.                        # meridional wind stress [m2.s-2]
    vstr_btm: float = 0.                        # meridional current stress at the bottom [m2.s-2]
    tflx_sfc: float = 0.                        # non-penetrative heat flux at the surface [K.m.s-1]
    tflx_btm: float = 0.                        # non-penetrative heat flux at the bottom [K.m.s-1]
    sflx_sfc: float = 0.                        # fresh water flux at the surface [psu.m.s-1]
    sflx_btm: float = 0.                        # fresh water flux at the bottom [psu.m.s-1]
    rflx_sfc_max: float = 0.                    # maximum solar radiation flux at the surface [K.m.s-1]
